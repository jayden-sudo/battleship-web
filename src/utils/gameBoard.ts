...
export class GameBoard {
  ...
  getBoardBin() {
    ...
    for (let i = 0; i < this._pos.length; i++) {
      if (
        this._pos[i].posStatus === PosStatus.ShipAttacked ||
  ...
  firedAt(shootAt: number, save = false): ShotResult {
    ...
    if (
      posStatus === PosStatus.EmptyAttacked ||
      posStatus === PosStatus.EmptyUnattacked
    ) {
      if (save) {
      ...
      if (this._pos[i].posStatus === PosStatus.ShipAttacked) {
        ...
        const uarr = [];
        {
          ...
          if (l >= 0 && l < this.size * this.size) {
            if (
              Math.floor(l / this.size) === line &&
              this._pos[l].posStatus === PosStatus.Unknown
            ) {
              uarr.push(l);
            }
          }
        }
        {
          ...
          if (r >= 0 && r < this.size * this.size) {
            if (
              Math.floor(r / this.size) === line &&
              this._pos[r].posStatus === PosStatus.Unknown
            ) {
              uarr.push(r);
            }
          }
        }
        {
          ...
          if (u >= 0 && u < this.size * this.size) {
            if (
              Math.floor(u / this.size) === line - 1 &&
              this._pos[u].posStatus === PosStatus.Unknown
            ) {
              uarr.push(u);
            }
          }
        }
        {
          ...
          if (d >= 0 && d < this.size * this.size) {
            if (
              Math.floor(d / this.size) === line + 1 &&
              this._pos[d].posStatus === PosStatus.Unknown
            ) {
              uarr.push(d);
            }
          }
  ...
  enemySaveShoot(shootAt: number, shotResult: ShotResult | null) {
    if (shotResult === null) {
      ...
      if (this._pos[shootAt].posStatus !== PosStatus.AttackedPending) {
        debugger;
        throw new Error(
          "Invalid state: position not in AttackedPending status",
        );
      }
      if (shotResult.shotStatus === FireStatus.STATUS_MISS) {
        ...
        if (_size > 2) {
          _size = _size / 6;
          for (
            let i = shotResult.sunkHeadPosition;
            i <= shotResult.sunkEndPosition;
            i += 6
          ) {
