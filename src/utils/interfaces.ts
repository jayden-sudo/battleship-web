...
export const ROUND_TIME_LIMIT = 15; // Seconds, from contract
export const REVEAL_RANDOMNESS_LIMIT = 5; // Seconds, from contract
...
export type P2PMessage =
    ...
    | { type: 'requestCreatorSignature'; data: { gameId: string; myWalletAddress: string; } }
    | { type: 'creatorSignature'; data: { endTime: number; signature: string; } }
    | { type: 'shot'; data: { statusHash: string; position: number; signature: string; } }
    | { type: 'report'; data: { statusHash: string; position: number; shotResult: ShotResult; signature: string; poof: string; } }
