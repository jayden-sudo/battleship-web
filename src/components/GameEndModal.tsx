...
export function GameEndModal({ isOpen, isWinner, onClose }: GameEndModalProps) {
  const [phase, setPhase] = useState<"zoom" | "shake" | "stable" | "fadeout">(
    "zoom",
  );
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            phase === "zoom"
              ? `radial-gradient(circle, ${mainColor}40 0%, transparent 50%)`
              : "transparent",
      ...
      <div
        ...
        style={{
          ...
          animation:
            phase === "shake" ? "koShake 0.1s ease-in-out infinite" : "none",
        }}
      >
      ...
      <style jsx>{`
        @keyframes koShake {
          ...
          25% {
            transform: translateX(-5px) rotate(-1deg) scale(1.1);
          }
          75% {
            transform: translateX(5px) rotate(1deg) scale(1.1);
          }
